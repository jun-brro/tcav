...
class GCAVSteering:
    ...
    def compute_concept_probability(self, activation: torch.Tensor, layer: int) -> torch.Tensor:
        ...
        cav = self.cavs[layer]
        w = cav['w'].to(self.device)
        b = cav['b'].to(self.device)
        
        # Flatten activation for dot product
        e_flat = activation.view(activation.size(0), -1)  # [batch, features]
    ...
    def compute_closed_form_epsilon(
        ...
        """
        if layer not in self.cavs:
            return torch.zeros(activation.size(0), device=self.device)
        ...
        target_logit = torch.logit(torch.tensor(target_prob, device=self.device))
    ...
    def apply_intervention(
        ...
        v = self.cavs[layer]['v'].to(self.device)
    ...
    def register_hooks(self, model, config: Dict):
        ...
        for layer_idx in self.target_layers:
            if layer_idx in self.cavs:
                ...
                if hasattr(model.language_model.model.layers, str(layer_idx)):
                    layer_module = getattr(model.language_model.model.layers, str(layer_idx))
